...
export default function PdfToWebpPage() {
  ...
  // Options
  const [scaleOpt, setScaleOpt] = useState(1.56)
  ...
  const handleConvert = async () => {
    if (!file) return
    setState('processing')
    setProgress({ stage: 'Compiling WebP via WebAssembly...', current: 0, total: 0 })
    ...
    try {
      ...
      const exportedResults = await exportPdfToImages(doc, file.name.replace(/\.pdf$/i, ''), {
        ...
        onProgress: (cur, tot) => {
          setProgress({ stage: `Compressing WebP page ${cur} of ${tot}...`, current: cur, total: tot })
        }
      })
      ...
      if (err instanceof PdfEngineError) {
        ...
        setErrorMsg('An unexpected error occurred while converting your PDF to WebP.')
      }
    }
  }
  ...
  const handleDownloadZip = async () => {
    ...
    try {
      ...
      downloadBlob(content, `${file.name.replace(/\.pdf$/i, '')}-webp-images.zip`)
    } catch (err) {
    ...
    >
      ...
      <div className="max-w-4xl mx-auto space-y-10 py-6">
        ...
        <div className="text-center space-y-3">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-violet-50 dark:bg-violet-950/50 text-violet-700 dark:text-violet-300 border border-violet-200 dark:border-violet-800">
            <Sparkles className="w-3.5 h-3.5 text-violet-600" />
            Next-Gen Image Format (30% Smaller than JPG)
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-zinc-900 dark:text-white tracking-tight">
            Convert PDF to WebP
          </h1>
          <p className="text-sm sm:text-base text-zinc-600 dark:text-zinc-400 max-w-2xl mx-auto">
            Convert PDF pages to Google WebP format for fast web delivery and superior compression without noticeable loss of quality.
          </p>
          <div className="pt-2">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 sm:p-8 shadow-sm">
          {state === 'idle' && (
            <DropZone
              ...
              label="Drop your PDF here to convert to WebP"
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-700">
                <div className="flex items-center space-x-3 truncate">
                  <div className="p-2.5 bg-violet-100 dark:bg-violet-900/30 text-violet-600 rounded-lg">
              ...
              <div className="border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 bg-zinc-50/50 dark:bg-zinc-800/20 space-y-4 text-left">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      <option value={1.0}>72 DPI — Screen / Web thumbnail</option>
                      <option value={1.56}>150 DPI — Standard WebP (Balanced)</option>
                      <option value={3.125}>300 DPI — High Density Print</option>
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full text-xs sm:text-sm bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 rounded-lg px-3 py-2 text-zinc-900 dark:text-white focus:ring-2 focus:ring-brand-500 outline-none"
                    >
                      <option value={0.75}>75% — Ultra small payload</option>
                      <option value={0.85}>85% — Optimal web distribution</option>
                      <option value={0.95}>95% — Maximum fidelity</option>
                    </select>
                  </div>
                </div>
            ...
            <div className="py-12 text-center space-y-4">
              ...
              <div className="space-y-1">
                <h3 className="text-base font-bold text-zinc-900 dark:text-white">
                  {progress.stage || 'Rendering WebP...'}
                </h3>
                <p className="text-xs text-zinc-500 dark:text-zinc-400">
                  Executing WebAssembly encoding on device.
                </p>
              </div>
            </div>
          )}
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-zinc-200 dark:border-zinc-800">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-500 dark:text-zinc-400">
                      {results.length} {results.length === 1 ? 'image' : 'images'} extracted
                    </p>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 text-left">
          <div className="space-y-3">
            ...
            <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">
              Google WebP compression outperforms both JPEG and PNG by delivering 25% to 34% smaller file sizes at identical visual index scores. Ideal for modern responsive web applications.
            </p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">
              With 85% to 95% quality modes, text remains perfectly readable without jagged artifacts or fuzzy edges around characters.
            </p>
          </div>
        </div>
      </div>
